...
=======
  private val isSSparkLogEnabled = sc.getConf.getBoolean("spark.ss.log.enabled", false)
  private val isSSparkProfileEnabled = sc.getConf.getBoolean("spark.ss.profile.enabled", false)
>>>>>>> LOSIC
...
=======
  ...
  private def submitStage(stage: Stage) {
    val jobId = activeJobForStage(stage)
    val isSSparkOptimizeEnabled = sc.getMutableConf("optimize")
...
=======
          //if (isSSparkProfileEnabled) {
          sc.makeLineage(jobId.get, stage.id, stage.rdd)  // SSPARK: make lineage on each Stage
          stage.rdd.setStageId(stage.id)
          val lineage = sc.getLineage(stage.id)
          
          /*lineage.foreach{ x =>
            stage.rdd.rddNames += (x._1 -> x._2.name)
          }*/
          val rootLineage = sc.rootLineage.last
          val (stageInputBytes, startType) = sc.getStageInputBytes(lineage, rootLineage)
          logInfoSSP(s"stage ${stage.id} input size = $stageInputBytes bytes from $startType", isSSparkLogEnabled)
          //}
          if (sc.isSSparkMemCheck) {
            sc.getRemainingStorageMem()
          }
          if (sc.isSSparkProfileEnabled) { // make profile planner
            sc.makeProfilePlanner(lineage, rootLineage, stageInputBytes)
          }
...
<<<<<<< HEAD
=======
              logInfoSSP(s"[Overlap mode] try to start selectCandidates thread...", isSSparkLogEnabled)
>>>>>>> LOSIC
...
=======
              logErrorSSP(s"""Wrong ss.optimize.mode: ${sc.optimizeMode} need "overlap" or "ahead"""")
            }
          }
>>>>>>> LOSIC
...
=======
      if(cur._1 == 0 && sc.isPairHadoopRDD(lineage(cur._1).name)) {
>>>>>>> LOSIC
...
=======
  private def handleComputedRdds(stage: Stage, lineage: LinkedHashMap[Int, sc.LineageInfo]): Unit = {
    import scala.collection.JavaConverters._

    val accum = stage.latestInfo.accumulables
    val computedRdds = new HashSet[Int]

    val accumComputedRdds = accum.find(x => x._2.name == Some(InternalAccumulator.COMPUTED_RDDS))
    accumComputedRdds match{
      case Some(x) =>
        if (x._2.value != None) {
          val getComputedRdds = x._2.value.get.asInstanceOf[java.util.List[Int]]
          logInfoSSP(s"reduced computedRdds from stage ${stage.id} $getComputedRdds", isSSparkLogEnabled)
          
          computedRdds ++= getComputedRdds.asScala
          logInfoSSP(s"computedRdds ${computedRdds}", isSSparkLogEnabled)
        }
        else
          logErrorSSP("Couldn't find computedRdds value")
      case _ => logInfoSSP(s"None computedRdds", isSSparkLogEnabled)  
    }
    sc.computedCount += computedRdds.size

    if (sc.isSSparkCountFunctional) {
      val count = computedRdds.count { rddId =>
        lineage.contains(rddId) && sc.functionalOps.contains(lineage(rddId).name)
      }
      sc.countFunctional += count
      logInfoSSP(s"functionalCount ${sc.countFunctional} (+$count)", isSSparkLogEnabled)
    }

    logInfoSSP(s"computedCount ${sc.computedCount} (+${computedRdds.size})", isSSparkLogEnabled)
  }
  /* 
  *  SSPARK: A function that calculates and merges the profiled data
  *  checked no overhead 27 Feb.
>>>>>>> LOSIC
  */
...
=======
      case _ => logInfoSSP(s"None accumBlockTime", isSSparkLogEnabled)  
    }
...
=======
      case _ => logInfoSSP(s"None accumBlockSize", isSSparkLogEnabled)  
    }
    ...
    if (rddTime.size > 0 || rddSize.size > 0)
      ...
      logInfoSSP(s"Nothing in rddTime:${rddTime} or rddSize:${rddSize}", isSSparkLogEnabled)
>>>>>>> LOSIC
...
<<<<<<< HEAD
=======
    // check and set hadoopRDDs size
    val missingKeys = rddTime.keySet.diff(rddSize.keySet).filter(sc.hadoopRDDs.contains(_))
    missingKeys.foreach { x =>
      rddSize.put(x, accumInputBytes)
    }
...
=======
      ...
      if (sc.isUncachableHadoopRDD(name)) {
        if (opTimePerIn.find(x => x._1 == nameWithCallSite) != None
          ...
          // uncacheableHadoopRDD cannot measure the size, so it forces the input size to be used as the output size.
          opOutPerIn.add( (nameWithCallSite, in.asInstanceOf[Long], in.asInstanceOf[Long]) )  // in, in
        }
      }
...
=======
          ...
          // it's not error case, if there are cached RDDs, then profile should be failed.
          logInfoSSP(s"rddProfile failed, name: $name, out: $out, in: $in, time: $time", isSSparkLogEnabled)
        }
     // }
    }
    ...
    def accumulateRddSize(rddId: Int, size: Double): Unit = {
      sc.rddSize(rddId) = math.max(sc.rddSize.getOrElse(rddId, Double.MinValue), size)
      val count = sc.countMap.getOrElse(rddId, 0) + 1
      sc.countMap(rddId) = count
              
      if (count >= 2) {
        sc.reuseRddSize(rddId) = math.max(sc.reuseRddSize.getOrElse(rddId, Double.MinValue), size)
      }
      /*
      if ( v > sc.rddSize.getOrElse(k, Double.MinValue)) {
        sc.rddSize(k) = v
        logInfoSSP(s"totalSize updated: ${sc.rddSize.values.sum} w/ $k $v", isSSparkLogEnabled)
      }*/
    }
...
=======
              (pId == 0 && sc.isPairHadoopRDD(x._2.name)) ) 
>>>>>>> LOSIC
...
=======
          /*
          if (sc.isUncachableHadoopRDD(x._2.name)) {
            logInfoSSP(s"visited uncachableRDD ${x._1} name:$name input:$in output:$out -> $in time:$time", isSSparkLogEnabled)  
            visited.add(x._1)
            rddProfileToOpProfile(name, in, in, time)
          }
          else {*/
            logInfoSSP(s"visited ${x._1} name:$name input:$in output:$out time:$time", isSSparkLogEnabled)
            visited.add(x._1)
            if (sc.isSSparkPrepareEnabled && out != None) {
              accumulateRddSize(x._1, out.asInstanceOf[Long].toFloat / Math.pow(2,20))
            }
            if (out != None && in != None && time != None) {
              sc.profiledCount += 1
              logInfoSSP(s"profiledCount++ ${sc.profiledCount}", isSSparkLogEnabled)
            }
            rddProfileToOpProfile(name, in, out, time)
          //}
>>>>>>> LOSIC
...
=======
    val isSSparkOptimizeEnabled = sc.getMutableConf("optimize")
    val unpersistPlan = sc.getMutableConf("unpersist")
...
<<<<<<< HEAD
=======
    //if (sc.lastUnpersistFlag && sc.lastOptimizeFlag)
    if (unpersistPlan && isSSparkOptimizeEnabled) {
      logInfoSSP(s"Optimize unpersist plan for ${stage.id}",isSSparkLogEnabled)
      sc.checkOptimizeUnpersist(stage.rdd)
    }
>>>>>>> LOSIC
...
=======
        ...
        handleComputedRdds(stage, lineage)
>>>>>>> LOSIC
      }
